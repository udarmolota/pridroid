...
//
// Built as libpridroid_exec.so (the "lib*.so" name is the Android packaging
...
//
// Usage:  libpridroid_exec.so <game_dir> <lib_dir> [extra RimWorld args...]
// Env (set by the launcher, inherited): BOX64_LD_LIBRARY_PATH, PRIDROID_RENDERER,
...
int main(int argc, char** argv) {
    ...
    // In the JNI app, PriDroidApplication calls System.loadLibrary("pridroidlinker"),
    // which puts libpridroidlinker.so in the DEFAULT linker namespace so that
    // launch_rimworld_elf()'s dlopen(RTLD_NOLOAD) can find it (load_linker_hook
    // separately loads it into the isolated pridroid_ns).  This exec'd process has
    // no Application, so do the default-namespace load here ourselves.
    void* linker = dlopen("libpridroidlinker.so", RTLD_NOW | RTLD_GLOBAL);
    if (!linker) {
        fprintf(stderr, "pridroid_exec: failed to load libpridroidlinker.so: %s\n", dlerror());
        return 3;
    }

    return pridroid_run_standalone(game_dir, lib_dir, gargc, gargv);
}
