...
#define LOG_TAG "pridroid-jni"

// Launch the standalone box64+RimWorld binary (libpridroid_exec.so) as a FRESH
...
// The binary lives next to libpridroid.so in nativeLibraryDir (resolved via
// dladdr; requires android:extractNativeLibs="true" so it's on disk + exec'able).
JNIEXPORT jint JNICALL
Java_com_pridroid_GameLauncher_execStandaloneGame(
        ...
        jstring j_game_dir, jstring j_lib_dir, jstring j_extra_arg)
{
    ...
    Dl_info info;
    if (dladdr((void*)&Java_com_pridroid_GameLauncher_execStandaloneGame, &info) && info.dli_fname) {
        strncpy(self_copy, info.dli_fname, sizeof(self_copy) - 1);
    }
    char* native_lib_dir = dirname(self_copy);   // mutates self_copy, returns dir
    char bin[4096];
    snprintf(bin, sizeof(bin), "%s/libpridroid_exec.so", native_lib_dir);
...
Java_com_pridroid_GameLauncher_startGame(
        JNIEnv* env, jobject clazz,
        ...
        jobjectArray j_args)
{
    ...
    pridroid_start_game(game_dir_path, library_dir_path, argc, (const char**)argv);
...
JNIEXPORT jint JNICALL
Java_com_pridroid_GameLauncher_initPriDroidWindow(JNIEnv* env, jobject clazz) {
    return pridroid_init();
}

JNIEXPORT void JNICALL
Java_com_pridroid_GameLauncher_destroyPriDroidWindow(JNIEnv* env, jobject clazz) {
    pridroid_deinit();
}

JNIEXPORT jint JNICALL
Java_com_pridroid_GameLauncher_setSurface(
        JNIEnv* env, jobject clazz,
        jobject surface, jint width, jint height)
{
    ANativeWindow* wnd = ANativeWindow_fromSurface(env, surface);
    pridroid_surface_init(wnd, width, height);
    return 1;
}

JNIEXPORT void JNICALL
Java_com_pridroid_GameLauncher_destroySurface(JNIEnv* env, jobject clazz) {
    pridroid_surface_deinit();
}

JNIEXPORT jint JNICALL
Java_com_pridroid_GameLauncher_nativeOsmesaSmokeTest(JNIEnv* env, jclass clazz, jstring jpath) {
    const char* path = jpath ? (*env)->GetStringUTFChars(env, jpath, NULL) : NULL;
    int rc = pridroid_osmesa_smoketest(path);
    if (path) (*env)->ReleaseStringUTFChars(env, jpath, path);
    return rc;
}

// --- Phase A input injection (touch → SDL mouse); rd_input_* declared in pridroid.h ---
// action: 0 = move, 1 = down, 2 = up. x,y are game-window pixels (already scaled).
JNIEXPORT void JNICALL
Java_com_pridroid_GameActivity_nativeTouch(JNIEnv* env, jclass clazz,
...
JNIEXPORT void JNICALL
Java_com_pridroid_GameActivity_nativeButton(JNIEnv* env, jclass clazz,
...
JNIEXPORT void JNICALL
Java_com_pridroid_GameActivity_nativeScroll(JNIEnv* env, jclass clazz,
...
JNIEXPORT void JNICALL
Java_com_pridroid_GameActivity_nativeKey(JNIEnv* env, jclass clazz,
...
JNIEXPORT void JNICALL
Java_com_pridroid_GameActivity_nativeText(JNIEnv* env, jclass clazz, jstring jtext) {
    if (!jtext) return;
...
Java_com_pridroid_GameActivity_nativeGetFrameCount(JNIEnv* env, jclass clazz) {
    return (jlong)g_pridroid_frame_count;
}

// Frame-rate cap: 0 = uncapped, else limit presents to this many FPS (pacing in
// pridroid_frame_tick). CPU-bound RimWorld benefits — steadier delivery + CPU
// freed for the tick loop.
extern volatile uint64_t g_pridroid_frame_min_ns;
JNIEXPORT void JNICALL
Java_com_pridroid_GameActivity_nativeSetFpsCap(JNIEnv* env, jclass clazz, jint fps) {
    g_pridroid_frame_min_ns = (fps > 0) ? (1000000000ull / (uint64_t)fps) : 0;
}
