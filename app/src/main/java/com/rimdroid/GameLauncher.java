...
public class GameLauncher {
    ...
    public static void launch(GameInstance gameInstance) throws ErrnoException {
        ...
        // Prison Architect's fixed-function OpenGL renderer. Do not inherit stale RimDroid choices
        ...
        Os.setenv("RIMDROID_TEX_SHRINK", "0", true);
        Os.unsetenv("RIMDROID_TEX_DEEP_MIN");
        ...
        switch (renderer) {
            case GL4ES:
                ...
                // X11-only video). The SDL_DYNAMIC_API stub path inherited from RimDroid exists
        ...
        // RIMDROID_EXEC override so rimdroid.c stays untouched. Placed AFTER the X11 block:
        // its smoke-test branch may point RIMDROID_EXEC at a diagnostic tool instead, and the
        // unsetenv branches above clear any stale override first — so null here means "the game".
        if (Os.getenv("RIMDROID_EXEC") == null) {
            Os.setenv("RIMDROID_EXEC",
