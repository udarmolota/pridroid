 ...
 * libpridroidlinker. In that process dlopen is the real bionic one, so the bundled
...
public class FmodDecodeService extends Service {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                File inst = (instanceName != null && !instanceName.isEmpty())
                        ? com.pridroid.AppStorage.requireSingleton().getInstanceDir(instanceName)
    ...
    private void notifyDone(boolean ok) {
        try {
            ...
            String text = getString(ok ? com.pridroid.R.string.sound_done : com.pridroid.R.string.sound_failed);
            ...
            android.app.Notification n = new android.app.Notification.Builder(this, ch)
                    ...
                    .setContentTitle(getString(com.pridroid.R.string.app_name))
                    .setContentText(text)
