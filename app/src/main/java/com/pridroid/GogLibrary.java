...
/**
 * Reading a signed-in user's GOG library (see docs/BRIEF_gog_downloader.md).
 *
 ...
 * predicting. An earlier version sent cookies only, on the theory that the listing was
 * cookie-authorised; GOG answered every call with
 * {@code {"status":403,"message":"Full authentication is required to access this resource."}} —
 * the OAuth resource-server refusal — because the WebView login leaves us holding galaxy-login-*
 * cookies scoped to login.gog.com and no www session at all.
 *
 ...
 * real response that way silently yields zero files (measured 2026-09-14: every product reported
 * {@code files=0} except one, which matched only because {@code extras} happens to share a name).
 * What GOG actually returns is described on {@link #details} — read it before changing this.
 *
 * <p>Note what we are NOT using: GOG Galaxy's content-system API (the chunked, depot-shaped one
 * that resembles our Steam downloader) serves {@code windows} and {@code osx} only. Linux builds
 * live exclusively on the older whole-file installer path used here.
 *
...
public final class GogLibrary {
    ...
    private static String userAgent() {
        ...
        try {
            ua = android.webkit.WebSettings.getDefaultUserAgent(PriDroidApplication.APP);
        } catch (Throwable t) {
    ...
    public static final class Product {
        ...
        /** The Linux installers, which is all PriDroid can actually use. */
    ...
    public static Product details(long productId) throws IOException {
        ...
        // Print what actually arrived. Three rounds of this feature were lost to assuming a shape;
        // one line of truth per product is cheaper than a fourth.
     ...
     * with a redirect to the signed CDN link, so we ask without following it and read Location.
     * That link is short-lived and account-bound: use it now, do not store or log it.
     */
    ...
    /** GET a JSON document, presenting every credential we have. */
    ...
    private static HttpURLConnection open(String url) throws IOException {
        ...
        // Only give up when we hold nothing at all. Requiring cookies here is what hid the
