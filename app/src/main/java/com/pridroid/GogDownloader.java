...
/**
 * Pulls one GOG installer onto the phone (see docs/BRIEF_gog_downloader.md).
 *
 ...
 * can reach {@link GogInstallerExtractor} that way — an extractor fed a truncated installer fails
 * in confusing places, and the user would have no idea the download was the problem.
 *
...
public final class GogDownloader implements Cancellable {
    ...
    public File download(GogLibrary.File file, File destDir, Listener listener) throws IOException {
        ...
        try {
            ...
            // is sending the whole file, so whatever we had is worthless and the part file restarts.
