...
import com.pridroid.xserver.render.FullscreenTransformation;
...
import com.pridroid.xserver.Property;
import com.pridroid.xserver.Visual;
import com.pridroid.xserver.Window;
import com.pridroid.xserver.WindowAttributes;
import com.pridroid.xserver.WindowManager;
import com.pridroid.xserver.XClient;
import com.pridroid.xserver.errors.BadAccess;
...
import com.pridroid.xserver.errors.BadWindow;
import com.pridroid.xserver.errors.XRequestError;
import com.pridroid.xserver.events.CreateNotify;
import com.pridroid.xserver.events.Event;
import com.pridroid.xserver.events.RawEvent;
...
public abstract class WindowRequests {
    ...
    public static void reparentWindow(XClient client, XInputStream inputStream, XOutputStream outputStream) throws XRequestError {
        int windowId = inputStream.readInt();
        int parentId = inputStream.readInt();
        short x = inputStream.readShort();   // PriDroid: position inside the new parent (was skipped)
        short y = inputStream.readShort();
    ...
    public static void getProperty(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        if ("WM_STATE".equals(atomName)) {
            Log.i("PriDroid/XServer", "GetProperty WM_STATE win=0x" + Integer.toHexString(window.id)
    ...
    public static void warpPointer(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        // PriDroid: use setPosition (NOT raw setX/setY) so the move triggers MotionNotify like a
    ...
    public static void setInputFocus(XClient client, XInputStream inputStream, XOutputStream outputStream) throws XRequestError {
        ...
        // PriDroid: the FOCUS WINDOW comes from the request body; revert-to is only the policy
    ...
    public static void getGeometry(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        android.util.Log.i("PriDroid/XServer", "GetGeometry win=0x" + Integer.toHexString(drawableId)
