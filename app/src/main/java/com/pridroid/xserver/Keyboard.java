...
public class Keyboard {
    ...
    // Two KeySyms are advertised for every core X11 keycode (unshifted + shifted).
    // The old allocation only had room for one per keycode even though every accessor indexes
    // this array as index * KEYSYMS_PER_KEYCODE. It happened to cover the low PC keycodes, then
    // silently zero-filled the upper half of GetKeyboardMapping's advertised table.
    ...
    /** Core X11 QueryKeymap reply: 256 key bits, indexed by the unsigned X keycode.
     *  SDL2 uses this to reconcile its held-key state. Returning an all-zero bitmap (the old stub)
     *  erased every synthetic W/A/S/D immediately after its KeyPress event, so clicks and wheel
     *  worked while camera movement could never remain held. Caller holds INPUT_DEVICE. */
    public byte[] snapshotPressedKeymap() {
        byte[] map = new byte[32];
        for (int i = 0; i < pressedKeys.size(); i++) {
            int keycode = pressedKeys.valueAt(i) & 0xFF;
            map[keycode >>> 3] |= (byte)(1 << (keycode & 7));
        }
        return map;
    }
    ...
    public boolean onKeyEvent(KeyEvent event) {
        // PriDroid: inline gamepad check (Winlator used its ExternalController helper);
