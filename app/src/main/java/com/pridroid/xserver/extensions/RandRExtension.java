...
/**
 * PriDroid addition (not from Winlator): MINIMAL RandR 1.3 — just enough for SDL2's
...
public class RandRExtension extends Extension {
            ...
            public void run() {
                for (int i = 0; i < 3; i++) {
                    ...
                    if (target == null || target.getOutputStream() == null) {
                        android.util.Log.w("PriDroid/XServer", "RANDR notify #" + (i + 1) + ": no live client, skipping");
                        continue;
                    }
                    try {
                        android.util.Log.i("PriDroid/XServer", "RANDR -> RRScreenChangeNotify " + w + "x" + h + " (#" + (i + 1) + ")");
                        target.sendEvent(new com.pridroid.xserver.events.RRScreenChangeNotify(
                                FIRST_EVENT, rootId, rootId, w, h, mw, mh, TIMESTAMP));
                    } catch (Exception e) {
                        android.util.Log.w("PriDroid/XServer", "RRScreenChange send err: " + e);
                    }
                }
            }
        }, "rd-randr-notify");
    ...
    public void handleRequest(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        android.util.Log.i("PriDroid/XServer", "RANDR req minor=" + opcode
        ...
        switch (opcode) {
            ...
            case ClientOpcodes.LIST_OUTPUT_PROPERTIES:
                ...
                break;
            default:
                android.util.Log.w("PriDroid/XServer", "RANDR: unsupported minor opcode " + opcode);
                client.skipRequest();
