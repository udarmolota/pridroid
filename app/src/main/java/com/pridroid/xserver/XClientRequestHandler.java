...
import com.pridroid.xconnector.ConnectedClient;
import com.pridroid.xconnector.RequestHandler;
...
import com.pridroid.xserver.requests.AtomRequests;
import com.pridroid.xserver.requests.CursorRequests;
import com.pridroid.xserver.requests.DrawRequests;
import com.pridroid.xserver.requests.ExtensionRequests;
import com.pridroid.xserver.requests.FontRequests;
import com.pridroid.xserver.requests.GrabRequests;
import com.pridroid.xserver.requests.GraphicsContextRequests;
import com.pridroid.xserver.requests.KeyboardRequests;
import com.pridroid.xserver.requests.PixmapRequests;
import com.pridroid.xserver.requests.SelectionRequests;
import com.pridroid.xserver.requests.WindowRequests;
...
public class XClientRequestHandler implements RequestHandler {
    ...
    private boolean handleNormalRequest(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException {
        ...
        // PriDroid: full request trace (PRIDROID_XTRACE) — names the request SDL/Unity 1.6 sends
        // right before it gives up, to find the 0-displays root cause. Off unless the property is set.
        if (TRACE) android.util.Log.i("PriDroid/XServer", "req seq=" + client.getSequenceNumber()
            ...
            if (opcode < 0) {
                ...
                switch (opcode) {
                    ...
                    case 32:   // UngrabKeyboard — no reply
                        ...
                        break;
                    default:
                        // PriDroid: never hang the client on an unimplemented request — skip it,
                        ...
                        android.util.Log.w("PriDroid/XServer", "Unsupported X opcode " + opcode
                                + " (seq " + client.getSequenceNumber() + ") — replying BadImplementation");
                        client.skipRequest();
                        new com.pridroid.xserver.errors.XRequestError(17, 0).sendError(client, opcode);
                        break;
                }
            }
        }
