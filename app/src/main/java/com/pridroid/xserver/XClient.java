...
import com.pridroid.xserver.util.Callback;
...
public class XClient extends ConnectedClient implements XResourceManager.OnResourceLifecycleListener {
    ...
    // PriDroid 1.6 OnGUI-spin hunt: is Unity's endless OnGUI fed by an X-event flood from US?
    ...
    public void sendEvent(Event event) {
        // PriDroid: MUST write under the stream lock — closing it flushes the buffer to the
        ...
        com.pridroid.xconnector.XOutputStream out = outputStream;
        if (out == null) return;
        try (com.pridroid.xconnector.XStreamLock lock = out.lock()) {
            ...
            synchronized (rd_eventCounts) {
                ...
                if (n <= 20 || n % 5000 == 0)
                    android.util.Log.i("PriDroid-XEvt", "sent #" + n + " last=" + type + " byType=" + rd_eventCounts);
            }
        }
        catch (IOException e) {
            e.printStackTrace();
