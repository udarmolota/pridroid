...
import com.pridroid.xserver.errors.BadMatch;
import com.pridroid.xserver.errors.BadValue;
import com.pridroid.xserver.errors.XRequestError;
import com.pridroid.xserver.events.ConfigureNotify;
import com.pridroid.xserver.events.ConfigureRequest;
import com.pridroid.xserver.events.DestroyNotify;
import com.pridroid.xserver.events.Event;
import com.pridroid.xserver.events.Expose;
import com.pridroid.xserver.events.MapNotify;
import com.pridroid.xserver.events.MapRequest;
import com.pridroid.xserver.events.ResizeRequest;
import com.pridroid.xserver.events.UnmapNotify;
...
public class WindowManager extends XResourceManager {
            ...
            public void run() {
                ...
                try {
                    for (int i = 0; i < 2; i++) {
                        ...
                        android.util.Log.i("PriDroid/XServer", "ConfigureNotify KICK #" + (i + 1)
                    ...
                    android.util.Log.w("PriDroid/XServer", "configure-kick failed (non-fatal)", e);
                }
            }
        }, "rd-configure-kick");
        t.setDaemon(true);
    ...
    public void mapWindow(Window window) {
        android.util.Log.i("PriDroid/XServer", "MAPWIN win=0x" + Integer.toHexString(window.id)
        ...
        if (!window.attributes.isMapped()) {
            ...
            if (!parent.hasEventListenerFor(Event.SUBSTRUCTURE_REDIRECT) || window.attributes.isOverrideRedirect()) {
                ...
                // PriDroid: a real X server also reports visibility after mapping — SDL2 tracks it.
                window.sendEvent(Event.VISIBILITY_CHANGE, new com.pridroid.xserver.events.VisibilityNotify(window));
    ...
    public void unmapWindow(Window window) {
        android.util.Log.i("PriDroid/XServer", "UNMAPWIN win=0x" + Integer.toHexString(window.id)
    ...
    public void setFocus(Window focusedWindow, FocusRevertTo focusRevertTo) {
        // PriDroid: deliver focus events — SDL2 waits for FocusIn after XSetInputFocus to mark
        ...
        if (old != null && old != rootWindow)
            old.sendEvent(Event.FOCUS_CHANGE, new com.pridroid.xserver.events.FocusOut(old));
        if (focusedWindow != null && focusedWindow != rootWindow)
            focusedWindow.sendEvent(Event.FOCUS_CHANGE, new com.pridroid.xserver.events.FocusIn(focusedWindow));
    }
    ...
    public void configureWindow(Window window, Bitmask valueMask, XInputStream inputStream) throws XRequestError {
        ...
        if (!parent.hasEventListenerFor(Event.SUBSTRUCTURE_REDIRECT) || overrideRedirect) {
            // PriDroid (RimWorld 1.6 / brief v12): suppress NO-OP ConfigureNotify. Unity's startup
            ...
            if (unchanged) {
                android.util.Log.i("PriDroid/XServer", "ConfigureWindow no-op (same geometry) win=0x"
    ...
    public void reparentWindow(Window window, Window newParent) {
        // PriDroid: full XReparentWindow semantics — a MAPPED window is unmapped, reparented, then
        ...
        android.util.Log.i("PriDroid/XServer", "reparent win=0x" + Integer.toHexString(window.id)
        ...
        window.sendEvent(Event.STRUCTURE_NOTIFY,
                new com.pridroid.xserver.events.ReparentNotify(window, window, newParent));
        newParent.sendEvent(Event.SUBSTRUCTURE_NOTIFY,
                new com.pridroid.xserver.events.ReparentNotify(newParent, window, newParent));
        if (oldParent != null && oldParent != newParent)
            oldParent.sendEvent(Event.SUBSTRUCTURE_NOTIFY,
                    new com.pridroid.xserver.events.ReparentNotify(oldParent, window, newParent));
        if (wasMapped) {
            updateWmState(window, WM_STATE_NORMAL);
            window.sendEvent(Event.STRUCTURE_NOTIFY, new MapNotify(window, window));
    ...
    private void updateWmState(Window window, int state) {
        ...
        if (property != null) {
            ...
            Log.i("PriDroid/XServer", "WM_STATE win=0x" + Integer.toHexString(window.id)
