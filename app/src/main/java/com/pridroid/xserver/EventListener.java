...
public class EventListener {
    ...
    public void sendEvent(Event event) {
        // PriDroid: write under the stream lock — closing it flushes to the socket. Without it
        ...
        com.pridroid.xconnector.XOutputStream out = client.getOutputStream();
        if (out == null) return;
        try (com.pridroid.xconnector.XStreamLock lock = out.lock()) {
            event.send(client.getSequenceNumber(), out);
            android.util.Log.i("PriDroid/XServer", "event -> " + event.getClass().getSimpleName());
        }
        catch (IOException e) {
            e.printStackTrace();
        }
