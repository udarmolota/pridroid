...
import com.pridroid.xserver.events.ButtonPress;
import com.pridroid.xserver.events.ButtonRelease;
import com.pridroid.xserver.events.EnterNotify;
import com.pridroid.xserver.events.Event;
import com.pridroid.xserver.events.KeyPress;
import com.pridroid.xserver.events.KeyRelease;
import com.pridroid.xserver.events.LeaveNotify;
import com.pridroid.xserver.events.MappingNotify;
import com.pridroid.xserver.events.MotionNotify;
import com.pridroid.xserver.events.PointerWindowEvent;
...
public class InputDeviceManager implements Pointer.OnPointerMotionListener, Keyboard.OnKeyboardListener, WindowManager.OnWindowModificationListener, XResourceManager.OnResourceLifecycleListener {
    ...
    public void onPointerButtonPress(Pointer.Button button) {
        // PriDroid: relative-mouse mode forwarded events to Wine's WinHandler in Winlator;
        ...
        if (!xServer.isRelativeMouseMovement()) {
            ...
            if (grabWindow != null && grabWindow.attributes.isEnabled()) {
                ...
                if (button.code() >= 4) {
                    android.util.Log.i("PriDroid/XServer",
    ...
    public void onPointerButtonRelease(Pointer.Button button) {
        // PriDroid: no Wine WinHandler — see onPointerButtonPress.
        if (!xServer.isRelativeMouseMovement()) {
            Bitmask eventMask = createPointerEventMask();
            Window grabWindow = xServer.grabManager.getWindow();
    ...
    public void onKeyPress(byte keycode, int keysym) {
        ...
        android.util.Log.i("PriDroid/XServer", "key route press code=" + (keycode & 0xFF)
    ...
    public void onKeyRelease(byte keycode) {
        ...
        android.util.Log.i("PriDroid/XServer", "key route release code=" + (keycode & 0xFF)
                + " focus=0x" + Integer.toHexString(focusedWindow.id)
                + " point=0x" + Integer.toHexString(pointWindow.id)
                + " event=0x" + Integer.toHexString(eventWindow.id)
                + " child=" + (child != null ? "0x" + Integer.toHexString(child.id) : "None"));
