...
/**
 * Process-wide queue of GOG downloads (see docs/BRIEF_gog_downloader.md).
 *
 * <p>It lives outside any screen for the reason {@link SteamDownloadState} does: a transfer of
 * hundreds of megabytes easily outlasts the fragment that started it, and the user must be able to
 ...
 * fragment used to have no idea a download was already running, and two writers appending to one
 * {@code .part} silently corrupt it.
 *
 * <p>Files download one at a time, in the order asked for: a game and its expansions are several
 * files over one connection, and splitting the bandwidth would only make each arrive later. The
 * keep-alive notification is held from the first request until the queue drains — one notification
...
public final class GogDownloadQueue {
    ...
    public synchronized void enqueue(Context ctx, GogLibrary.File file, File destDir) {
        ...
        if (!running) {
            ...
            holdKeepAlive(true);
            new Thread(this::drain, "rd-gog-download").start();
        }
        changed();
    }
     ...
     * five seconds after it leaves the screen — device log, 2026-09-15: "Destroyed live tcp sockets
     * for uids={10770}" 5.0 s after switching away, then our read died with "Software caused
     * connection abort". Always called with the queue's lock held, so the stop that ends one batch
     * can never overtake the start of the next.
     */
